...
-- Update get_hunter_form_data to include budget and hiring_model but exclude client
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'job', (SELECT row_to_json(j) FROM (
      SELECT id, title, description, budget, hiring_model FROM public.jobs WHERE id = v_job_id
    ) j),
